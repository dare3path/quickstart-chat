...
pub fn send_message(ctx: &ReducerContext, text: String) -> Result<(), String> {
    ...
    log::info!("Sending msg from '{}' time:{} msg:'{}'", ctx.sender, ctx.timestamp, text);
...
// also called via: spacetime call quickstart-chat send_message 'Hello, World1!'
pub fn client_connected(ctx: &ReducerContext) {
    if let Some(user) = ctx.db.user().identity().find(ctx.sender) {
        ...
        log::info!("Making new user({}) and setting as online!", ctx.sender);
